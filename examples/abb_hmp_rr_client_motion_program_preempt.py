# Example Robot Raconteur client using "motion_program" and preempting the motion program
#
# The "motion_program" execution mode uses the vendor controller motion computer. It works by sending a list
# of motion programs to execute to the control, and then using commands on the controller to execute the motion.
# "Preemption" allows the client to provide a new list of commands, and switch to that new command list at a specified
# command number. This example demonstrates using the Robot Raconteur driver to preempt a motion. See
# abb_motion_program_exec for more information on how preemption works.
...
mp_p1_cmds = []
mp_p1_cmds.append(movej(r3,0.5,0.02,False))
mp_p1_cmds.append(movec(r4,r5,0.2,0.01,False))
mp_p1_cmds.append(movec(r4,r3,0.05,0.01,False))

mp_p1 = motionprogram_type()
mp_p1.motion_program_commands = mp_p1_cmds
mp_p1.extended = {
    "first_command_number": RR.VarValue(5, "uint32")
}
...
with suppress(RR.StopIterationException):
    while True:
        res = mp_gen.Next()
        print(f"res.status: {res.action_status}, res.cmd: {res.current_command}, res.queued: {res.queued_command}, res.preempt: {res.current_preempt}")

        if res.current_command > 2 and not preempt_sent:
            preempt_sent = True
            c.preempt_motion_program(mp_p1, 1, 4)
